...
export function blankProject(T, name, client, em, target) {
  const checklist = T.checklist.map(c => ({ id: c.id, section: c.section, subsection: c.subsection, item: c.item, owner: '', due: '', status: '', notes: '', useCase: '' }));
  ...
  return {
    id: uid('P'), name, client, em, phase: 'Discovery', targetDate: target, createdAt: Date.now(), updatedAt: Date.now(),
    checklist, plan, raciNames: {}, raciMarks, baseline, useCases: [], kpis: [], hypotheses: [], stakeholders: [], library: [],
    discoveryNotes: '', discoveryCandidates: [],
